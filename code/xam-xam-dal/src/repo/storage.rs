...
pub fn update_storage(conn : &PgConnection, id_user : i32, storage_name : &str, update_model : &UpdateStorage) -> Result<bool, XamXamError> {
    ...
    if update_model.name.is_none() && update_model.storage_kind.is_none() {
        info!("Storage {} of user {} could not be updated, because the update model was empty",storage_name,id_user);
        return Ok(false)
    }
    match diesel::update(storages.filter(user_id.eq(id_user)).filter(name.eq(storage_name))).set(update_model).execute(conn) {
        Ok(rows_affected) => {
            if rows_affected > 0 {
                info!("Storage {} of user {} has been updated",storage_name,id_user);
                Ok(true)
            } else {
                info!("Storage {} of user {} could not be updated",storage_name,id_user);
                Ok(false)
            }
        },
        Err(e) => Err(e.into())
...
pub fn delete_storage(conn : &PgConnection, id_user : i32, storage_name : &str) -> Result<bool, XamXamError> {
    ...
    match diesel::delete(storages.filter(user_id.eq(id_user)).filter(name.eq(storage_name))).execute(conn) {
        Ok(rows_affected) => {
            if rows_affected > 0 {
                info!("Storage {} of user {} has been deleted",storage_name,id_user);
                Ok(true)
            } else {
                info!("Storage {} of user {} could not be deleted",storage_name,id_user);
                Ok(false)
            }
        },
        Err(e) => Err(e.into())
