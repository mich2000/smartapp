...
pub fn get_information_from_id(conn: &PgCon, user_id: i32) -> Result<BasicUserInfo, XamXamError> {
    ...
    let result : BasicUserInfo = diesel::sql_query("select count(distinct s.id) as amount_storage, count(pi.id) as amount_product, min(pi.peremption_date) as min_bederf, max(pi.peremption_date) as max_bederf from storages s left join products pi on pi.storage_id = s.id where s.user_id = $1")
