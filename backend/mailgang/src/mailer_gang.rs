...
impl Default for Mailer {
    ...
    fn default() -> Self {
        ...
        Mailer(get_transport(&domain, &email, &password), email.to_owned())
    }
}

impl Mailer {
    pub fn new(domain: &str, email: &str, pwd: &str) -> Mailer {
        Mailer(get_transport(domain, email, pwd), email.to_owned())
    }

    pub fn test_mail_to_self(&self) -> Result<(), &'static str> {
        self.send_email(Report::new(
            &self.1.clone(),
            "",
            "Test mail",
            "Server test",
    ...
    pub fn send_email(&self, report: Report) -> Result<(), &'static str> {
        ...
        match self
            .0
            .lock()
            .expect("Could not lock the email transport")
            .send(mail.into())
        {
            Ok(_) => {
            ...
            Err(err) => {
                warn!(
                    "The email could not be send this is related to the smtp transport. error: {}",
                    err
                );
