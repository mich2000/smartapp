...
*/
pub fn introduce_user_creation_demand(
    redis_conn: &mut RCon,
    db_conn: &PgCon,
    mailer: &Mailer,
    email: &str,
    ...
    if r2d2_redis::redis::cmd("EXISTS")
        .arg(email)
        .query::<bool>(redis_conn.deref_mut())?
    {
    ...
    mailer.send_email(Report::new(
        email,
        "",
        "Token for user account creation",
        &clear_text,
    )?)?;
    Ok(())
}
...
pub fn create_user(
    ...
    let email: Option<String> = match redis::cmd("GET")
        .arg(model.get_token().to_owned())
        .query(redis_conn.deref_mut())
    {
        Ok(res) => res,
        Err(e) => {
    ...
    user::insert_user(
        db_conn,
        &InsertableUser::new(model.get_email(), model.get_password())?,
    )?;
    Ok(())
}
...
pub fn change_email(
    ...
    db_conn: &PgCon,
    user_id: i32,
    model: &NewEmailHolder,
) -> Result<(), XamXamServiceError> {
    if model.get_token().len() != 4 {
        return Err(XamXamServiceError::TokenHasNotCorrectLength);
    }
    let email = redis::cmd("GET")
        .arg(model.get_token())
...
pub fn change_pwd(
    ...
    let person: User = user::get_user_by_mail(db_conn, email)?
        .ok_or_else(|| XamXamServiceError::from(XamXamError::UserIsNotPresent))?;
...
*/
pub fn send_token_forgotten_pwd(
    redis_conn: &mut RCon,
    db_conn: &PgCon,
    mailer: &Mailer,
    model: &EmailHolder,
    ...
    mailer.send_email(Report::new(
        model.get_email(),
        "",
        "Token to change forgotten password",
        &clear_text,
    )?)?;
    Ok(())
}
...
pub fn change_forgotten_pwd(
    ...
    let person: User = user::get_user_by_mail(db_conn, model.get_email())?
        .ok_or_else(|| XamXamServiceError::from(XamXamError::UserIsNotPresent))?;
...
*/
pub fn authenthicate_get_token(
    db_conn: &PgCon,
    claim_config: &ClaimConfiguration,
    email: &str,
    pwd: &str,
) -> Result<String, XamXamServiceError> {
    if pwd.is_empty() {
        return Err(XamXamError::PasswordIsEmpty.into());
    }
    let person: User = user::get_user_by_mail(db_conn, email)?
...
pub fn renew_token(
    ...
    Ok(claim_config.token_from_claim(
        &claim_config.create_claim(&claim_config.decode_token(token)?.claims.get_subject())?,
    )?)
}
...
*/
pub fn get_basic_information(
    db_conn: &PgCon,
    user_id: i32,
) -> Result<BasicUserInfo, XamXamServiceError> {
...
pub fn get_five_first_products(
    conn: &PgCon,
    user_id: i32,
) -> Result<ProductUnits, XamXamServiceError> {
